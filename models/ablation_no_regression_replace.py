...
def multi_osESD_supervised_no_replace(x_data, y_data, params, train_percent, testing_size):
...
def multi_osESD_supervised_no_replace_del(x_data, y_data, params,train_percent):
    ...
    for train_epoch in range(epochs):
        ...
        for i in range(train_online_len, len(x_data) - init_size):
            ...
            '''
            Getting rid of replacing mechanism
            '''
            # if row_pred == 1:
            #     skip_next = False
            #     for single_anom_idx, single_anom_val in enumerate(row_vals):
